...
function MenuRadioItem({
  ...
  return (
    <MenuPrimitive.RadioItem
      className={cn(
        "grid min-h-8 in-data-[side=none]:min-w-[calc(var(--anchor-width)+1.25rem)] cursor-default items-center rounded-sm py-1 text-base text-foreground outline-none data-disabled:pointer-events-none data-highlighted:bg-accent data-highlighted:text-accent-foreground data-disabled:opacity-64 sm:min-h-7 sm:text-sm [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        compact ? "relative grid-cols-[1fr] px-2" : "grid-cols-[1rem_1fr] gap-2 ps-2 pe-4",
        className,
      )}
    ...
    >
      <MenuPrimitive.RadioItemIndicator
        className={cn(compact ? "pointer-events-none absolute left-2 top-1/2 -translate-y-1/2" : "col-start-1")}
      >
        <svg
          fill="none"
          height="24"
      ...
      <span className={cn(!compact && "col-start-2")}>{children}</span>
    </MenuPrimitive.RadioItem>
  );
}
